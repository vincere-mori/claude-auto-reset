...
3. Автоматом повесь расписание (см. ниже) или настрой руками. На Windows по умолчанию **4 ч 50 мин**; Linux/macOS в примере крона — **каждые 4 ч** на целый час (можно поменять руками).
...
Пауза при исчерпании лимита: логика во вложенной папке **`windows/internal/`**, трогать вручную не нужно — вызывает только `ping-claude.bat`. Если пришло сообщение с лимитом (в т.ч. строка `resets …`), задача в **Планировщике** может **быть перестроена на одно срабатывание** ближе ко времени сброса, а после **успешного** пинга снова вызывается `schedule-install` и возвращается интервал **каждые 4 ч 50 мин** (Windows: `schtasks` с шагом **290 минут**). Если правка задачи в Windows не удалась — всё равно действует файл-пауза в `%LOCALAPPDATA%`, просто без сдвига триггера.
...
Создаётся задача **`claude-auto-reset`**, раз в **4 часа 50 минут** (290 мин), под тем юзером, от которого ты запустил установку (`schtasks`). Если ошибка доступа - тот же `schedule-install` из cmd **от администратора** или задачу вручную в `taskschd.msc`.

Обычный «пинг» без аргумента - как раньше (одноразово).

**Linux / macOS** (шаблон в `cron-install`: раз в **4 часа** на целый час — так проще одной строкой в crontab; это **не** те же 4 ч 50 мин, что в Windows; при необходимости подставь свой интервал вручную):
...
Планировщик заданий: каждые **4 ч 50 мин** (или эквивалент **290 минут**) запуск **`cmd.exe /c "полный путь к ping-claude.bat"`**.
